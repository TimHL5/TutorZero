...
const STRIPE_YEARLY_PRICE_CENTS = 7999;
const STRIPE_PRODUCT_NAME = "TutorZero Pro";
...
app.post("/api/subscription/checkout", authMiddleware, async (c) => {
  ...
  try {
    ...
    const priceCents = isYearly ? STRIPE_YEARLY_PRICE_CENTS : STRIPE_MONTHLY_PRICE_CENTS;
    ...
    switch (event.type) {
      ...
      case "invoice.payment_succeeded": {
        ...
        if (invoice.billing_reason === "subscription_cycle") {
          ...
          // Extend subscription by 1 month on successful renewal
          const expiresAt = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString();
